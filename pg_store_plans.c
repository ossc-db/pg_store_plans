...
static const uint32 PGSP_FILE_HEADER = 0x20211106;
...
static void store_entry(char *plan, queryid_t queryId,
 ...
 */
static void
store_entry(char *plan, queryid_t queryId,
		   double total_time, uint64 rows,
		   const BufferUsage *bufusage)
{
	EntryKey key;
						...
						pgspVersion api_version)
{
	...
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		...
		if (is_allowed_role || entry->key.userid == userid)
		{
			...
			{
				/*
				 * For backward compatibility with v1.5, display the queryid
				 * for the queryid_stat_statements field too.
				 */
